...
@foreach ($categorias as $categoria)
 <option value="{{ $categoria->id }}" {{ $categoria->id == $categoriaFilho->id_categoria ? 'selected' : null}}>{{ $categoria->name }}</option>
@endforeach
</select>
</div>
...
<input type="text" name="name" placeholder="Add nome da categoria" class="form-control" value="{{ old('name', $categoriaFilho->name) }}">
</div>
...
$('.categoria-gerente').append(`<option value="${item.id}">${item.name}</option>`);
})
},
